...
Module Main
    ...
    Sub Main()
        ...
        Catch Ex As Exception
            MessageBox.Show("A critical error has occured. Please copy this message (Ctrl+C) and report it at https://github.com/gmedina-exporim/Exporim-File-Sync/issues :" & Environment.NewLine & Environment.NewLine & Ex.ToString, "Critical error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Throw
        End Try
    End Sub
End Module
...
#Region "Scheduling"
    Private Function SchedulerAlreadyRunning() As Boolean
        Dim MutexName As String = "[[Exporim File-Sync scheduler]] " & Application.ExecutablePath.Replace(ProgramSetting.DirSep, "!"c).ToLower(Interaction.InvariantCulture)
